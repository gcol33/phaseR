...
// Forward declaration
double phase_log_posterior_re(NumericVector params, List data,
                              int n_trans_coef, int n_dyn_coef_0, int n_dyn_coef_1,
                              int n_trans_re, int n_dyn_re_0, int n_dyn_re_1,
                              bool has_trans_re, bool has_dyn_re_0, bool has_dyn_re_1);

// Numerical gradient for RE model
NumericVector numerical_gradient_re(
    NumericVector params,
    List data,
    int n_trans_coef,
    int n_dyn_coef_0,
  ...
  for (int i = 0; i < n; i++) {
    ...
    double f_plus = phase_log_posterior_re(params_plus, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                            n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                            has_trans_re, has_dyn_re_0, has_dyn_re_1);
    double f_minus = phase_log_posterior_re(params_minus, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                             n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                             has_trans_re, has_dyn_re_0, has_dyn_re_1);
...
// Leapfrog step for RE model
void leapfrog_step_re(
    NumericVector& q,
    NumericVector& p,
    List data,
    int n_trans_coef,
  ...
  NumericVector grad = numerical_gradient_re(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                              n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                              has_trans_re, has_dyn_re_0, has_dyn_re_1);
  for (int i = 0; i < n; i++) {
    p[i] += 0.5 * step_size * grad[i];
  ...
  grad = numerical_gradient_re(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                has_trans_re, has_dyn_re_0, has_dyn_re_1);
  for (int i = 0; i < n; i++) {
    p[i] += 0.5 * step_size * grad[i];
...
List phaseR_nuts_sampler_re(
  ...
  NumericVector q = clone(init);
  double step_size = 0.05;  // Smaller initial step size for RE models
  ...
  for (int iter = 0; iter < n_iter; iter++) {
    ...
    double current_H = -phase_log_posterior_re(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                                n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                                has_trans_re, has_dyn_re_0, has_dyn_re_1);
    for (int i = 0; i < n_params; i++) {
      current_H += 0.5 * p[i] * p[i];
    ...
    int n_steps = std::max(1, static_cast<int>(1.0 / step_size));
    n_steps = std::min(n_steps, 50);  // Fewer steps for RE models

    for (int step = 0; step < n_steps; step++) {
      leapfrog_step_re(q_proposal, p_proposal, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                       n_trans_re, n_dyn_re_0, n_dyn_re_1,
                       has_trans_re, has_dyn_re_0, has_dyn_re_1, step_size);
    }

    double proposed_H = -phase_log_posterior_re(q_proposal, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                                 n_trans_re, n_dyn_re_0, n_dyn_re_1,
                                                 has_trans_re, has_dyn_re_0, has_dyn_re_1);
    for (int i = 0; i < n_params; i++) {
      proposed_H += 0.5 * p_proposal[i] * p_proposal[i];
