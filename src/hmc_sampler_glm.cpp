...
// Forward declaration
double phase_log_posterior_glm(NumericVector params, List data,
                                int n_trans_coef, int n_dyn_coef_0, int n_dyn_coef_1,
                                int family_0, int family_1,
                                int n_trials_0, int n_trials_1);

NumericVector numerical_gradient_glm(
  ...
  for (int i = 0; i < n; i++) {
    ...
    double f_plus = phase_log_posterior_glm(params_plus, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                             family_0, family_1, n_trials_0, n_trials_1);
    double f_minus = phase_log_posterior_glm(params_minus, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                              family_0, family_1, n_trials_0, n_trials_1);
...
void leapfrog_step_glm(
  ...
  NumericVector grad = numerical_gradient_glm(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                               family_0, family_1, n_trials_0, n_trials_1);
  for (int i = 0; i < n; i++) {
  ...
  grad = numerical_gradient_glm(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                 family_0, family_1, n_trials_0, n_trials_1);
  for (int i = 0; i < n; i++) {
    p[i] += 0.5 * step_size * grad[i];
  }
...
List phaseR_nuts_sampler_glm(
  ...
  for (int iter = 0; iter < n_iter; iter++) {
    ...
    double current_H = -phase_log_posterior_glm(q, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                                 family_0, family_1, n_trials_0, n_trials_1);
    for (int i = 0; i < n_params; i++) {
    ...
    n_steps = std::min(n_steps, 100);

    for (int step = 0; step < n_steps; step++) {
      leapfrog_step_glm(q_proposal, p_proposal, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                        family_0, family_1, n_trials_0, n_trials_1, step_size);
    }

    double proposed_H = -phase_log_posterior_glm(q_proposal, data, n_trans_coef, n_dyn_coef_0, n_dyn_coef_1,
                                                  family_0, family_1, n_trials_0, n_trials_1);
    for (int i = 0; i < n_params; i++) {
